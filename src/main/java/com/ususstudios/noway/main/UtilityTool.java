...
public class UtilityTool {
    ...
    public static JSONObject getJsonObject(String filePath) {
        ...
        try (InputStream inputStream = UtilityTool.class.getResourceAsStream(filePath)) {
            ...
            if (inputStream == null) {
                Game.LOGGER.warn("From: getJsonObject(), \"{}\" is not a valid path", filePath);
                return null;
            }

            // Warn if the filePath does not contain ".json", meaning it's not a JSON file
            if (!filePath.endsWith(".json")) {
                Game.LOGGER.warn("From: getJsonObject(): \"{}\" does not lead to a json file", filePath);
                return null;
            }
    ...
    public static String[] getFileNames(String directoryPath) {
        ...
        try (InputStream inputStream = UtilityTool.class.getResourceAsStream(directoryPath)) {
            ...
            if (inputStream == null) {
                Game.LOGGER.warn("From: getFileNames(): \"{}\" is not a valid path.", directoryPath);
                return null;
            }
